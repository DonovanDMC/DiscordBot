...
const replacements: Array<[RegExp, (match: string, ...args: Array<string>) => string]> = [
    ...
    // [[title]]
    [/\[\[([\S ]+?)(?:\|[\S ]+)?]]/gi, (_match: string, title: string) => `<${config.baseURL}/wiki_pages/show_or_new?title=${title.replaceAll(" ", "_")}>`],
    // {{tags}}
    [/{{([\S ]+?)(?:\|[\S ]+)?}}/gi, (_match: string, tags: string) => `<${config.baseURL}/posts?tags=${tags.replaceAll(" ", "%20")}>`]
];
